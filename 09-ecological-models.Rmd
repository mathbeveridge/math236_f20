...
I have written some code that generates and plots a trajectory for a Leslie system. You should run this code chunk before the others.
...
get_leslie_trajectory <- function(L, start, N) {
...
plot_leslie_trajectory <- function(X, agegroups) {
  ...
  # Plot graph 
  plot(t,X[1,],type='l',col=1,ylim=c(0,ylimit),ylab="population",xlab="time (year)", main="Population in Age Group")
  ...
  legend(N *1.1,ylimit * .85,agegroups, col=1:m, lty = 1)
...
X = get_leslie_trajectory(L, start, N)

plot_leslie_trajectory(X, c("(0-1)", "(1-2)", "(over 2)"))
...
plot_leslie_trajectory(X, c("Fingerlings (0-1)", "Yearlings  (1-2)", "Young Adults (2-3)","Adults (3-4)","Adults (4-5)"))
