...
One measure of centrality is to use the **degree** of the vertex, or the number of edges connected to that vertex. We calculate it this way: (1) First make a vector $\mathsf{v}$ of all 1's; then (2) multiply $\mathsf{d} = A\mathsf{v}$ to get the degree proportions; and (3) we also divide vector $\mathsf{d}$ by the sum of its entries. This creates a "normalized vector" $\mathsf{u}$ whose entries sum to 1. Each entry of vector $\mathsf{u}$ represents to proportion of edges incident with the corresponding vertex.
...
The Gould Index is a measure of centrality that uses the dominant eigenvector of a matrix. It was introduced by geographer P. R. Gould in 1967 to analyze the geographical features on maps. We will build up Gould's Index step-by-step so that we can understand what it measures. 
...
Now let's plot the network with the vertices sized by the Gould Index.

```{r, network6, fig.height=6, fig.width=6, eecho=TRUE}
plot(g, layout=locations, vertex.size=250*u,vertex.label.cex=0.65, vertex.color='tan1', vertex.frame.color="dodgerblue" )
```

And we can create a data frame containing both the degree centrality and the Gould Index. We  order the data using the Gould Index and then compare the two.
```{r, echo=TRUE}
...
**Discuss with your group:** Degree centrality and Gould's Index give different rankings. 
...
Remind yourselves what does the Gould Index measures. 
