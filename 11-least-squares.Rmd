...
**The Why**: Given a matrix $A$ and a vector $\mathsf{b}$ that is not in $W = \mathrm{Col}(A)$, we want to find the "best approximate solution" $\hat{\mathsf{b}} \in W$. In other words, we want to pick the best possible $\hat{\mathsf{b}} \approx \mathsf{b}$ that is in the column space of $A$.
...
Here are some points that we'd like to fit a line to. 
...
To fit a line $y = d + cx$ to the data, we need  to solve the following $Ax = b$ problem:
$$
\begin{bmatrix} 
1 & 1 \\  1 & 2 \\ 1 & 3 \\ 1 & 4 \\
\end{bmatrix}
...
These equations are inconsistent, so we solve the normal equations $A^T A x = A^T y$ instead. A clever way to create the desired matrix $A$ is to use the fact that $x^0=1$ for any number $x$.
...
It might surprise you at first that we can use *linear algebra* to fit a *quadratic function*. But remember: we are fitting the **coefficients**. One we plug in our experimental data, we have a linear combination of the unknown coefficients!
...
* Remove the leading tick mark in the line below make the block executable.
...
$$f = c e^{k(y-1750)},$$
where $c$ and $k$ are the unknown constants (parameters of the model). How do we fit a linear model in this case? Just take the logarithm of both sides! Doing so yields the following linear system:
$$\log(f)=\log(c)+k(y-1750).$$
...
* Use least squares approximation to find the best fitting exponential function for this data. This will give you the values for $d$ and $k$, and once you know $d$, you can find $c = exp(d)$. We have started the code for you by defining `x=year-1750` and `y=log(emissions)`

```{r, echo=TRUE}

### your code goes here. 
# be sure to define d and k
...
* Run the code below to plot the original data along with your exponential model curve $f(y)$.


  + **Note:** This code assumes that you have already defined the values for `k` and `c`. Otherwise, it will not work!
  + Remove the leading tick mark in the line below make the block executable.  

'```{r, echo=TRUE}
f=function(y){exp(a*exp(k*(y-1750))}
