...
a. Assume that $A$ is a  $4 \times 4$ matrix such that Gaussian Elimination to  REF only uses two types of Elementary Row Operations: 
   ...
   Explain why $C$ has the the form
    ...
    where the diagonal elements $c_{11}, c_{22}, c_{33}, c{44}$ are nonzero.
...
**Remark 2:** If Gaussian Elimination of $A$ requires (E2) exchange two rows then we can choose to do those row swaps first. This corresponds to multiplying $A$ by a **permutation matrix** $P$. We then perform Gaussian Elimination on $PA$: we can factor $PA=LU$ where $L$ is lower triangular and $U$ is upper triangular. This is called the **$PLU$-decomposition** of $A$. 

**Remark 3:** It turns out that explicitly calculating $A^{-1}$ can lead to some very problematic rounding errors. So most solvers use the $LU$-decomposition to solve $A \mathsf{x} = \mathsf{b}$ instead of calculating $\mathsf{x} = A^{-1}\mathsf{b}$. More specifically, we replace $A \mathsf{x} = \mathsf{b}$ with $LU \mathsf{x} = \mathsf{b}$ and then solve this in two steps.
...
I heard someone say that transaltions via homogeneous coordinates will help to solve my problem. But I'm not sure what to do. Help!
