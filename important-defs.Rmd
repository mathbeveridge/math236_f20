...
$$
If $A = [\mathsf{v}_1 \mathsf{v}_2 \cdots \mathsf{v}_n]$ is the matrix with these vectors in the columns, then this is the same as saying that $\mathsf{x} = [c_1, \ldots, c_n]^{\top}$ is a solution to $A x  = \mathsf{v}$.
...
$$
is with $c_1 = c_2 = \cdots = c_n = 0$. This is the same as saying that $A x = \mathsf{0}$ has only the trivial solution.
...
\mathsf{0} = c_1 \mathsf{v}_1 + c_2 \mathsf{v}_2 + \cdots + c_n \mathsf{v}_n
$$
This is called a **dependence relation** among the vectors. This is the same as saying that $\mathsf{x} = [c_1, c_2, \ldots, c_n]^{\top}$ is a nontrivial solution to $A \mathsf{x} = \mathsf{0}$.
...
: A function $T: \mathbb{R}^n \to \mathbb{R}^m$ is a **linear transformation** when: 
    ...
    * $T(c \mathsf{u} ) = c T(\mathsf{u})$ for all $\mathsf{u} \in \mathbb{R}^n$ and $c \in \mathbb{R}$.

It follows from these that also $T(\mathsf{0}) = \mathsf{0}$ (preserves scalar multiplication).
