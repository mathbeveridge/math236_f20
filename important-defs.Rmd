...
: A square $n \times n$ matrix $P$ is orthogonal when is columns are an orthonormal set. As a result, we have $P^{-1} = P^{\top}$.
...
: A symmetric matrix $A$ can be written a linear combination of rank 1 matrices derived from the orthonormal eigensystem of $A$. In particular, we have
$$
...
* $\Sigma$ is a matrix whose nonzero entries are the positive numbers $\sigma_1, \ldots , \sigma_r$ are in decreasing order on the diagonal, and 
...
: Any $m \times n$ matrix $A$ of rank $r$ can be written a linear combination of rank 1 matrices derived from the singular value decomposition of $A$. In particular, we have
$$
